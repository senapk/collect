...
# Definir a função para executar comandos Git
...
def pull(directory: str) -> Text:
    ...
    # 1. Reset Local Hard
    output += Text("y") + "Local "
    stdout, stderr, returncode = run_git_command(directory, ["reset", "--hard", "HEAD"])
    if returncode != 0:
    ...
    # 2. Clean
    output += Text("y") + "Clean "
    stdout, stderr, returncode = run_git_command(directory, ["clean", "-fd"])
    if returncode != 0:
    ...
    # 3. Fetch
    output += Text("y") + "Fetch "
    stdout, stderr, returncode = run_git_command(directory, ["fetch", "origin"])
    if returncode != 0:
    ...
    stdout, stderr, returncode = run_git_command(directory, ["reset", "--hard", f"origin/{main_branch}"])
    if returncode != 0:
...
def main():
    ...
    if args.folders:
        pull_all_parallel(args.folders, n_threads)
